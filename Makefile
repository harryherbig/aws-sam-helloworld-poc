...
init: sam-build-container sam-deploy-all ## run when no cloudformation stack exists

destroy: s3-empty ## deletes sam cf stack via aws cli after emptying the s3 bucket
...
invoke-hook-local: sam-build-container ## locally invoke PreLiveHook with Test Event (needs aws credentials)
	sam local invoke --profile default PreLiveHook --event events/prelivehook.json

invoke-function-local: sam-build-container ## locally invoke HelloWorldFunction with SQS Test Event
